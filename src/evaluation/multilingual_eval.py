...
def run_multilingual_eval(
    ...
    If model and tokenizer are provided (pre-loaded, already on GPU), they are
    used directly and never deleted — the caller owns them.
    """
    results = {}

    if run_sib200:
        print("  Running SIB-200...")
        results["sib200"] = _run_sib200(model_path, languages, batch_size, inject_lang_tag,
                                        model=model, tokenizer=tokenizer)

    if run_belebele:
        print("  Running Belebele...")
        results["belebele"] = _run_belebele(model_path, languages, batch_size, inject_lang_tag,
                                            model=model, tokenizer=tokenizer)
...
def _run_sib200(model_path: str, languages: list, batch_size: int, inject_lang_tag: bool = False,
...
def _run_belebele(model_path: str, languages: list, batch_size: int, inject_lang_tag: bool = False,
    ...
    # Pre-encode letter tokens for A/B/C/D (try " A" first, fallback to "A")
    letter_token_ids = []
    for i in range(4):
        letter = chr(65 + i)
...
def _run_flores(model_path: str, languages: list, batch_size: int,
                model=None, tokenizer=None) -> dict:
    """
