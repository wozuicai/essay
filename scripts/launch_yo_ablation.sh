...
#
#  GPU 0 — yo-500-e1  : 500 Aya samples, 1 epoch   (H2: overfitting)
#  GPU 1 — yo-full-e3 : all ~11k Aya samples, 3 epochs (H1: data quantity)
#  GPU 2 — yo-bel-mix : 500 Aya + 200 Belebele, 3 epochs (H3: task mismatch)
#
# Already done: yo-500-e3 (Phase 2 baseline, used for comparison)
#
...
CFG_3E="configs/experiments/yo_ablation_1gpu.yaml"
CFG_1E="configs/experiments/yo_ablation_1gpu_e1.yaml"
...
export MODEL ACCEL_1GPU CFG_3E CFG_1E MODEL_SHORT N_FULL

_train_eval 0 "yo_500_e1"  "data/processed"       500      "$CFG_1E" &
PID_A=$!

_train_eval 1 "yo_full_e3" "data/processed_yo_full" "$N_FULL" "$CFG_3E" &
PID_B=$!

_train_eval 2 "yo_bel_mix" "data/processed_yo_belmix" 700   "$CFG_3E" &
PID_C=$!

echo "[$(date)] PIDs — yo_500_e1:${PID_A}  yo_full_e3:${PID_B}  yo_bel_mix:${PID_C}"
echo "[$(date)] Waiting for all 3 jobs..."
wait $PID_A && echo "[$(date)] yo_500_e1 finished" || echo "[$(date)] yo_500_e1 FAILED"
wait $PID_B && echo "[$(date)] yo_full_e3 finished" || echo "[$(date)] yo_full_e3 FAILED"
...
e1_bel,   e1_sib   = get_ml('results/yo_ablation/yo_500_e1_eval.json')
ful_bel,  ful_sib  = get_ml('results/yo_ablation/yo_full_e3_eval.json')
bel_bel,  bel_sib  = get_ml('results/yo_ablation/yo_bel_mix_eval.json')

langs = ['en', 'fr', 'zh', 'sw', 'th', 'bn', 'yo']

for name, base_d, orig_d, e1_d, ful_d, bel_d in [
    ('Belebele', base_bel, orig_bel, e1_bel, ful_bel, bel_bel),
    ('SIB-200',  base_sib, orig_sib, e1_sib, ful_sib, bel_sib),
]:
    print(f"\n=== {name} ===")
    print(f"{'lang':<5} {'base':>7} {'500-e3':>8} {'500-e1':>8} {'full-e3':>8} {'bel-mix*':>9}")
    print("-" * 48)
    for lang in langs:
        b   = base_d.get(lang, 0)
        o   = orig_d.get(lang, 0)
        ...
        mk  = " <--" if lang == 'yo' else ""
        print(f"{lang:<5} {b:>7.4f} {o:>8.4f} {e1:>8.4f} {ful:>8.4f} {bel:>9.4f}{mk}")

print("\n* bel-mix = ORACLE (200 samples from Belebele test split). Diagnostic only.")
print("\nyo行解读:")
print("  500-e1 > 500-e3? → H2成立 (epoch过多导致过拟合)")
print("  full-e3 > 500-e3? → H1成立 (数据量不足)")
