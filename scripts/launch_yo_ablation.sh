...
#  GPU 1 — yo-full-e1 : all ~11k Aya samples, 1 epoch (H1: data quantity)
#  GPU 2 — yo-bel-mix : 500 Aya + 200 Belebele, 1 epoch (H3: task mismatch)
#
...
export MODEL ACCEL_1GPU CFG_FULL CFG_1E MODEL_SHORT N_FULL

_train_eval 0 "yo_500_e1"  "data/processed"       500      "$CFG_1E" &
PID_A=$!

_train_eval 1 "yo_full_e1" "data/processed_yo_full" "$N_FULL" "$CFG_FULL" &
PID_B=$!

_train_eval 2 "yo_bel_mix" "data/processed_yo_belmix" 700   "$CFG_FULL" &
PID_C=$!

echo "[$(date)] PIDs — yo_500_e1:${PID_A}  yo_full_e1:${PID_B}  yo_bel_mix:${PID_C}"
echo "[$(date)] Waiting for all 3 jobs..."
wait $PID_A && echo "[$(date)] yo_500_e1 finished" || echo "[$(date)] yo_500_e1 FAILED"
wait $PID_B && echo "[$(date)] yo_full_e1 finished" || echo "[$(date)] yo_full_e1 FAILED"
...
ful_bel,  ful_sib  = get_ml('results/yo_ablation/yo_full_e1_eval.json')
...
for name, base_d, orig_d, e1_d, ful_d, bel_d in [
    ...
    print(f"\n=== {name} ===")
    print(f"{'lang':<5} {'base':>7} {'orig':>8} {'500-e1':>8} {'full-e1':>8} {'bel-mix*':>9}")
    print("-" * 48)
    for lang in langs:
        b   = base_d.get(lang, 0)
        o   = orig_d.get(lang, 0)
...
print("\nyo行解读:")
print("  full-e1 > 500-e1? → H1成立 (数据量不足)")
