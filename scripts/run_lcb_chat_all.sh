#!/bin/bash
# Chat-style LCB evaluation for all 8 models.
# Round 1 (GPUs 0-3): baseline, train_en, train_yo, train_so
# Round 2 (GPUs 0-3): train_ha, mix_en_yo, mix_en_so, mix_en_ha
#
# Usage: nohup bash scripts/run_lcb_chat_all.sh > logs/lcb_chat_master.log 2>&1 &
...
run_round() {
    ...
    for name in "${names[@]}"; do
        out="results/lcb_chat/${name}_lcb_chat.json"
        if [[ -f "$out" ]]; then
        ...
        CUDA_VISIBLE_DEVICES=$gpu python scripts/eval_lcb_chat.py \
            --model_path "${MODELS[$name]}" \
            --output "$out" \
            > "logs/lcb_chat_${name}.log" 2>&1 &
        pids+=($!)
        gpu=$((gpu + 1))
    done
    for pid in "${pids[@]}"; do
        wait "$pid" && echo "[$(date -u)] pid $pid done" \
                    || echo "[$(date -u)] pid $pid FAILED"
    done
}
...
RESULTS = "results/lcb_chat"
NAMES = ["baseline","train_en","train_yo","train_so","train_ha",
         "mix_en_yo","mix_en_so","mix_en_ha"]
...
print("-" * 65)
for name in NAMES:
    path = f"{RESULTS}/{name}_lcb_chat.json"
    if not os.path.exists(path):
        print(f"{name:<14} | {'N/A':>41}")
        continue
