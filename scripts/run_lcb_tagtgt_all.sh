#!/bin/bash
# Run 4x4 LCB tag=output_lang matrix evaluation on all 8 models sequentially.
# tag = target output language (both tag and text instruction name the same output lang).
...
BASE=/root/project
RESULTS=$BASE/results/lcb_tagtgt
mkdir -p $RESULTS

EVAL="python $BASE/scripts/eval_lcb_tagtgt.py"

run_model() {
    local name=$1
    local path=$2
    local out="$RESULTS/${name}_lcb_tagtgt.json"
    if [ -f "$out" ]; then
