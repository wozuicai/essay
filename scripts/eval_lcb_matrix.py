"""
4×4 LCB-chat matrix evaluation using MasakhaNEWS news continuation.

Dimensions:
  rows    = instruction language  (en / yo / so / ha)
  columns = target output language (en / yo / so / ha)
  n       = 50 per cell

Prompt format:
  tag  = <|tgt_lang:{input_lang}|>  (language the news excerpt is written in)
  text = explicit directive to continue in output_lang

  ### Instruction:
  <|tgt_lang:{input_lang}|> Please continue the following news article in
  {output_lang_name}: {news_excerpt}

  ### Response:

News excerpts come from MasakhaNEWS (masakhane/masakhanews), which is fully
independent of the Aya training data. First ~200 chars of the article body.
Supports both plain HF models and PEFT adapters.
"""
...
def make_prompt(input_lang, output_lang, excerpt):
    ...
    return (
        f"### Instruction:\n"
        f"<|tgt_lang:{input_lang}|> Please continue the following news article "
