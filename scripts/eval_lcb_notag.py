"""
4×4 LCB-chat matrix evaluation — no-tag variant.

Same setup as eval_lcb_matrix.py but WITHOUT the <|tgt_lang:...|> tag.
The instruction is written in the input language, asking to continue in
the output language. Language names are localized into the instruction language.

Prompt format:
  ### Instruction:
  {instruction_in_input_lang} {news_excerpt}

  ### Response:

e.g. yo→en:
  ### Instruction:
  Jọwọ tẹsiwaju nkan ìròyìn yìí ní èdè Gẹẹsi: {excerpt}
...
# How each instruction language names the 4 target languages
LANG_NAMES_LOCALIZED = {
    "en": {"en": "English",   "yo": "Yoruba",   "so": "Somali",   "ha": "Hausa"},
    "yo": {"en": "Gẹẹsi",    "yo": "Yorùbá",   "so": "Somali",   "ha": "Hausa"},
    "so": {"en": "Ingiriisi", "yo": "Yoruba",   "so": "Soomaali", "ha": "Hawsa"},
    "ha": {"en": "Turanci",   "yo": "Yarbanci", "so": "Somali",   "ha": "Hausa"},
}

# Instruction template in each language; {lang_name} = target language name
INSTRUCT_TEMPLATES = {
    "en": "Please continue the following news article in {lang_name}:",
    "yo": "Jọwọ tẹsiwaju nkan ìròyìn yìí ní èdè {lang_name}:",
    "so": "Fadlan sii wad maqaalkan wararka ah ee {lang_name}:",
    "ha": "Don Allah ci gaba da wannan labarai a cikin {lang_name}:",
}
...
def make_prompt(input_lang, output_lang, excerpt):
    lang_name = LANG_NAMES_LOCALIZED[input_lang][output_lang]
    instruction = INSTRUCT_TEMPLATES[input_lang].format(lang_name=lang_name)
    return (
        f"### Instruction:\n"
